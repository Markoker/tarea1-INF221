...
lld** transponerMatriz(lld** a, int n, int m)
{
    lld** b = new lld*[m];
    for (int i = 0; i < m; i++)
        b[i] = new lld[n];
...
lld** multiplicacionTranspuesta(lld** a, lld** b, int n, int l, int m)
{
    lld** c = new lld*[n];
    for (int i = 0; i < n; i++)
        c[i] = new lld[m];

    b = transponerMatriz(b, l, m);
...
int main(int argc, char const *argv[])
{
    ...
    while (std::getline(infile, line)) {
        ...
        lld** c = multiplicacionTranspuesta(a, b, n1, n2, n4);
